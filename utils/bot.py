...
def draw_eyes():
    ...
    pygame.draw.rect(
        ...
        border_radius=ref_corner_radius,
    )
    # Draw right eye
    pygame.draw.rect(
        screen,
...
# Function to display streaming text
def display_streaming_text(text, color=GREEN, delay=0.1):
    screen.fill(BLACK)  # Clear the screen
    text_surface = font.render(text, True, color)
    text_width, text_height = text_surface.get_size()
    x = (SCREEN_WIDTH - text_width) // 2  # Center horizontally
    y = (SCREEN_HEIGHT - text_height) // 2  # Center vertically

    for i in range(len(text)):
        screen.fill(BLACK)  # Clear the screen for the streaming effect
        current_text = text[:i + 1]
        rendered_text = font.render(current_text, True, color)
...
while running:
    ...
    if demo_mode == 1:
        # Alternate between animations and text display
        if current_animation_index == 0:
            display_streaming_text("Hello World!")
        else:
